...
export function SearchDropdown({ onClose, fullWidth = false }: Props) {
  ...
  const goToProduct = useCallback((p: Product) => {
    navigate(`/store/${p.category}?q=${encodeURIComponent(p.name)}`);
    closeAndClear();
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Escape') {
      ...
      results[activeIdx] ? goToProduct(results[activeIdx]) : submitSearch();
    }
  };
    ...
    <div ref={containerRef} className="relative">
      <div className={pillClasses}>
        ...
        <input
          ...
          onBlur={() => setTimeout(() => { setIsFocused(false); setOpen(false); setActiveIdx(-1); }, 150)}
        />
        {query && (
          <button
