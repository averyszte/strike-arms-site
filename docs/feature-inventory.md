...
| D2.1 | Dashboard reads every order unpaged | MISSING | `listAllOrdersWithItems()` selects with no range, so PostgREST's max-rows cap silently truncates it and **revenue quietly under-reports** once orders pass the limit — the same class of silent money error as the archive bug in D5.4. Not urgent (roughly a year of trading away) and not fixed here because doing it properly means a shared pager between it and `listOrdersForExport`, which already pages correctly. |
