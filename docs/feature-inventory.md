...
| A4 | **Site search** | DONE 2026-08-30 **[needs db push]** | The florist has no search at all — category then filters only. **The dropdown downloaded the catalogue and ranked it in the browser, capped at 500 rows** — fine at 56 products, and at 501 it stops being able to see the tail with nothing anywhere saying so. Migration 015 moves both search paths into the database: a generated `search_text` column (name, brand, blurb **and tags**, lowercased), a pg_trgm GIN index so a leading-wildcard `like` is not a sequential scan, and a `search_products` RPC that ranks server-side using the same weighting the browser used — name prefix, then name, brand, tags, blurb, with stock as the tiebreak. `security invoker`, so RLS applies as the calling role, and `is_published` pinned as well because an admin passes the read-all policy and must not get drafts in the public search box. The limit is clamped in SQL: an anonymous caller must not be able to turn a search box into a catalogue download. **The two searches now agree** — `/store?q=` filters on the same `search_text` column, so a product the dropdown offers on a tag match is one "View all results" actually lists; before, the dropdown searched tags and the store page did not. **Separately, `buildOrdersQuery` interpolated the admin’s raw search string into a PostgREST `.or()`** — a customer called "Smith, John" would not error, it would silently become two more filter clauses; now escaped like the product one always was. No `/search` route: search results are `/store?q=`, already noindexed with its own title, and a second URL for the same grid is a duplicate — `_redirects` sends `/search` there for people who type it. |
| A5 | **Brands hub** `/brands` | DONE 2026-08-30 **[alan]** | Hub cards and its ItemList now point at real `/brands/:slug` pages: breadcrumb, ItemList schema, canonical, shelf links carrying the brand as a query parameter, and load-more. A brand with no published products 404s rather than rendering an empty page. Copy is templated from the catalogue on purpose -- **[alan]** is only for real per-brand copy, which can be dropped in later without touching the page. |
...
| C6.2 | Admin UI for the rates | DONE | 2026-08-30. `/admin/settings` edits the flat delivery charge, the free-delivery threshold and the VAT rate. Changing the delivery rate no longer means a SQL edit, so C6 is now a question for Alan rather than a task for me. Three things worth knowing. (1) The UPDATE policy on `store_settings` is `is_admin_aal2()`, so a session that has not passed TOTP matches no row and PostgREST returns **success having changed nothing** — the admin would watch a "Saved" toast while the rates stayed as they were. `updateStoreRates` therefore reads the row back and throws a message naming two-factor verification. The read-back is safe here only because this table's select policy is `using (true)`; the same trick on `inquiries` would break every real visitor, because its select policy is `is_admin()`. (2) Amounts are validated as digit strings, not with `Number()`: `Number('1e3')` is 1000 and would have set a €1,000 delivery charge from a pasted value. Euros convert with `Math.round(value * 100)` — 6.50 is 649.9999… in binary, and truncating undercharges by a cent on every order forever. (3) The mutation seeds the cache with `setQueryData` rather than invalidating, because `useStoreRates` has a 10-minute staleTime and an admin who saves should not watch the shop quote the old price for ten minutes. VAT is still labelled as extracted from inclusive prices and still unconfirmed by an accountant — see C7. |
...
| C8 | **Refunds** | PARTIAL **[needs deploy]** | Admin-initiated refunds built 2026-08-30 (see D5.1). Still deliberately does not restock: a refunded item may have been damaged, kept, or never collected, so putting it back on sale is a decision, not a consequence. No credit note document yet -- that belongs with C7. |
...
| D2 | **Dashboard home** | DONE 2026-08-30 | Stats cards + recent orders + delivery pipeline. Will be empty until real orders exist. Archived orders count towards revenue and volume but not the queues (D5.4). **Now splits on `channel`**: a "By channel, same period" breakdown under the revenue chart, net of refunds, sorted biggest first. A blended figure flattered the website -- if most of the month was rung up at the counter, the shop is doing well and the site is doing nothing, and one number could not tell Alan which. The split windows through the same `periodStart()` the chart uses (one `PERIOD_DAYS` constant), because a split that does not add up to the total beside it reads as a bug in the money; verified by fixture that it sums to the chart exactly. Channels with no orders are omitted rather than shown as zero rows. |
| D2.1 | Dashboard reads every order unpaged | DONE 2026-08-30 | Both halves of `listAllOrdersWithItems()` were unpaged, so PostgREST's max-rows cap silently truncated them and **revenue under-reported** once orders passed the limit. The export was truncating too: `listOrdersForExport` paged its orders but its line-item fetch only chunked by 100 ids and never paged *within* a chunk, and 100 orders can carry more lines than the row cap — a dropped line is a product that silently sells none. One shared pager in `data/orders-bulk-reads.ts` (`pageAll` + `listOrderItemsFor`, `ID_CHUNK` moved there) now backs the dashboard, the export's orders and the export's items. **`pageAll` requires the caller to ask for `count: 'exact'`** — the count is how the loop knows it is finished, and without it it stops at the first short page, which is the exact bug it exists to prevent. Verified against fixtures: 1,200 rows behind a 200-row cap return all 1,200. |
| D3 | **Product management** | DONE 2026-08-30 | CRUD, image upload (A7), bulk publish/feature/delete, and CSV export/import. Import matches rows on Slug, so editing a slug in the spreadsheet creates a second product rather than renaming one -- creates are counted apart from updates in the preview so that is visible before it happens. Only columns present in the file are written, so a Slug+Price file changes prices and nothing else; an absent column never blanks a field. Stock is exported but never imported, because stock moves through adjust_stock which records who and why -- rows whose Stock cell disagrees are counted and reported. No transaction is possible over PostgREST, so the outcome names each row the database refused and re-running the same file is safe. |
...
| D5.1 | Admin-initiated refund | DONE 2026-08-30 **[needs deploy]** | New Edge Function `refund-order`, plus a Refund section on the order sheet. Two gates: `verify_jwt = true` so the gateway rejects anything without a Supabase session, then `is_admin_aal2()` run **as the caller** -- asking with the service key would answer a different question. The order row is **not** written here. `charge.refunded` reaches the webhook, which calls `record_refund`, and that stays the only writer of refund state -- so a refund taken in the Stripe dashboard and one taken in the admin are recorded by the same code path instead of two that can disagree. The cost is a few seconds where Stripe has accepted it and the badge has not caught up; the toast says so, because an unchanged badge otherwise reads as a failure and invites a second press. The amount is recomputed server-side from the row and an over-refund is **refused, not clamped** -- quietly refunding less than was typed is how a customer ends up short. One idempotency key per opened dialog, so a double-click is one refund and a deliberate second refund is a new one. Euro parsing rejects `1e3` and rounds the scaled value. Needs `supabase functions deploy refund-order`, and `charge.refunded` must be in the Stripe endpoint's event list (the webhook already handles it). |
| D5.2 | Packing slip / invoice print | DONE 2026-08-30 | Two documents at `/admin/orders/:id/print` (`?doc=invoice` for the second), opened in a new tab from the order sheet. The route sits outside `AdminLayout` so a printed page carries no sidebar -- never rendering the chrome beats guessing at print CSS -- but still inside `AuthGuard`, because an order is not public. It does not auto-print: on a shared machine that quietly sends a customer address to whatever printer was last used. The packing slip carries no prices and splits a mixed order into "In this parcel" and "Held for collection -- do not post", with a tick box per line. The invoice shows VAT as *included in* the total, not added to it, since site prices are gross. **Open:** a compliant Irish VAT invoice also needs the seller VAT registration number, which we do not hold -- deliberately not stubbed, so Alan must supply it before these go out as VAT invoices rather than receipts. |
...
| D6 | **Inquiries** | DONE 2026-08-30 | The Contact page now has a form. **`createInquiry` would have failed for every real visitor**: it did `.insert().select().single()`, but anon is granted insert and nothing else and the select policy is `is_admin()`, so reading the row back would have worked perfectly while logged in to test it and errored for the public. Returns void now. **Closed the loop that made this pointless:** nothing emails Alan (E3 deferred), so the dashboard alert card gained `N unanswered enquiries` linking to `/admin/inquiries` — otherwise messages land in a screen nobody has a reason to open. `buildOperationalAlerts` now takes a named object rather than four positionals. **Spam:** honeypot (off-screen, not `display:none`, out of the tab order, `autocomplete=off` so a password manager cannot fill it and silently drop a real customer) plus a 3-second minimum fill time. A caught submission gets the same thank-you as a real one, because a bot told it failed just tries again differently. Both are client-side and a targeted bot walks through them — **C11 is still needed**; these stop the drive-by spam that makes an enquiries screen unreadable. GDPR consent is a required checkbox. The thank-you promises a reply in a day or two and offers the phone, rather than "check your inbox", because no email is sent. Validation and bot checks verified against fixtures. **Untested:** the anon insert has not been run against the live database. |
...
| D9 | **Inventory adjustments** | DONE 2026-08-30 | Nothing called `adjust_stock` -- the RPC, the table and `inventory-repository.ts` all existed with no caller. A dialog on each product row now does In/Out + units + a required reason, and lists the last eight adjustments. **The floor is the reserved count, not zero**: the RPC adds a negative straight through zero, and units between `reserved_count` and zero are already promised to somebody mid-payment, so removing them oversells an open checkout. Quantity is checked as a digit string, not via `Number()`, which accepts `1e3` as a whole number and would book a thousand units. `adjusted_by` is now passed from the client (it defaulted to null, so every adjustment recorded against nobody); **the proper fix is a migration making the function read `auth.uid()` itself** -- deferred, needs a DB change. `ProductsTable` was split to `ProductsTableRow` first, at 246 lines. |
| D10 | **Gift card admin** | MISSING **[no ref]** | Only if C10 ships. |
| D11 | **Operational alerts** | DONE 2026-08-30 | The row said MISSING; a card already existed with two hardcoded rules and, more to the point, **no way to reach the orders it was complaining about** — it said "review in Orders" in link-coloured text that was not a link, which is still making someone go looking. Now `lib/admin-alerts.ts` builds a typed list and every row is a link to the filtered screen that fixes it. Seven rules, critical first then by count: failed payments; **paid, cancelled and never refunded** (real money owed back, and D5.1 means it has to be done by hand — the alert says so); paid and untouched over 24h; packed but not shipped over 2 days (the customer has a confirmation and a parcel that has not moved); ready for collection over a week (shelf space and stock the website cannot sell); live products out of stock; live products down to 2 or fewer. Fully-reserved counts as out of stock — sellable is `stockCount - reservedCount`. Thresholds are named constants, guesses to be argued with after Alan has run a month on it. **Deliberately not one alert per product/order** — an alert that fires constantly is one nobody reads. To make the links land, the orders list now takes its filters from the query string (`?payment=`, `?fulfillment=`) rather than local state, so a filter is addressable and cannot disagree with the address bar; both values are validated against the statuses that exist, not cast. The toolbar gained a fulfilment select to match. Rules verified against fixtures, not by eye. **Not done:** no badge in the sidebar — alerts are computed on the dashboard only, so you still have to be on it. |
| D12 | Tap-to-call / tap-to-email in detail sheets | DONE 2026-08-30 | `ContactLinks` in the order and enquiry sheets. Email prefills a subject — the order number, or `Re:` the enquiry subject — because the alternative is typing "your order" into a phone while a customer waits. **Not trivial in one respect:** a field filled in by hand holds things that are not numbers. `085 123 4567 ext 2` stripped of punctuation dials an eleven-digit stranger, and `085 1234567 / 01 4567890` dials nobody, so anything containing a letter, under 7 digits or over 15 (E.164) stays plain text. Malformed emails likewise. A link that looks live and does nothing is worse than text, because you tap it, nothing happens and you blame the phone. Numbers are **not** normalised to +353 — guessing a country code invents information, and `085...` dials fine from an Irish handset, which is the one behind the counter. **Deliberately not in the list views:** the whole row already opens the record, so a link inside it would swallow the tap and start a call when someone meant to read the order. Rows are 44px tap targets on mobile. The public site already had tap-to-call for the shop's own number. |
...
| E1.1 | GRANT audit | DONE 2026-08-30 **[needs db push]** | Migration 014. The florist's failure mode — a correct policy with no GRANT, giving a silent 401 — **we did not have**: every table the browser touches is granted and policied consistently, including the contact form insert. The faults were the mirror image, and worse. **`adjust_stock` was SECURITY DEFINER, had no revoke and no authorisation check at all**: the anon key is public, so anyone could `POST /rest/v1/rpc/adjust_stock` with `p_adjustment: -999` and rewrite stock on any product, writing whatever they liked into the audit log — which also recorded whoever the caller *said* they were, via `p_adjusted_by`. Now requires an AAL2 admin and attributes the row to `auth.uid()`. **`reserve_stock`** (superseded by `reserve_order_stock`, called by nothing since) was likewise public: anyone could inflate `reserved_count` until a product showed out of stock. **`checkout_reservations` anon INSERT** was unused *and* a route to overselling — the sweeper decrements `reserved_count` for any past-dated row, so a forged row released other people's genuine holds five minutes later. Policy dropped, grant revoked. Finally `admins`, `stripe_event_log` and `notification_jobs` were granted to `authenticated` directly beneath a comment saying they were not; RLS with no policies is all that was holding them. The migration also records what was checked and found correct, so the next audit does not start over. **Not applied — needs `supabase db push`.** |
...
| F7 | **T&C / Returns / Shipping policy** | **MISSING [alan] [legal]** | Legally required for EU e-commerce (14-day withdrawal right). Cannot launch a shop without these. **Now blocking a little:** the contact form's consent line cannot link to a privacy policy because there is not one. |
| F8 | **Best-of listicles** (5 drafted) | MISSING **[alan]** | Written, unrouted. |
| F9 | **Sitemap** | DONE 2026-08-30 | `scripts/generate-sitemap.mjs` reads Supabase. The old script regexed slugs out of `mock-products.ts`, which was accurate only by coincidence — the database was seeded from that file, so the two agreed exactly and would have gone on agreeing until the first time Alan added, renamed or unpublished anything. Regenerating now returns an **identical 71-URL set**, which is the proof it was reading the right data and the point at which it would have started drifting silently. Now: published products and their `<lastmod>` come from the DB; brand URLs are derived from the brands that actually have published stock (the old list came from `brands.ts`, which could list a brand whose `/brands/:slug` page 404s); missing env or a failed query exits non-zero rather than writing a stale file. Run `pnpm --filter @workspace/strike-arms run sitemap` after catalogue changes — still manual, and a Cloudflare Pages Function would make it automatic. |
...
| G1.1 | `_headers` — CSP and security headers | DONE 2026-08-30 | `public/_headers` written: CSP, HSTS preload, `X-Frame-Options: DENY`, nosniff, referrer policy, a Permissions-Policy that denies everything we do not use, immutable caching on `/assets/*`, no-cache on the shell, and `X-Robots-Tag: noindex` on cart, checkout, account and admin. `worker-src` is omitted deliberately — nothing here starts a worker. `style-src` carries `'unsafe-inline'` because Radix and framer-motion set style attributes; `script-src` does not. The Supabase origin is written out in full rather than wildcarded, so it must be changed alongside `VITE_SUPABASE_URL`. **Unverified until the first deploy** — a CSP can only really be tested against the live site. |
| G1.2 | `_redirects` ordering | DONE 2026-08-30 | The SPA fallback now targets `/` rather than `/index.html`, which is the extensionless form the florist lesson calls for. The old-site 301s already sit above it and shadow nothing. **Verify on the first deploy:** load a deep link such as `/store/rifles` and reload on it. If the rewrite misbehaves, deleting the rule falls back to Pages serving the shell for unmatched paths by itself. |
...
| G8 | **Verify migrations are actually applied** | DONE 2026-08-30 **[needs db push]** **[florist]** | Migration 016 adds `applied_migrations()` -- admin-only, returns the versions the Supabase CLI recorded. `scripts/generate-migration-manifest.mjs` writes `src/lib/migration-manifest.ts` from `supabase/migrations/*.sql` and runs as part of `build`, so the repo side of the comparison is the deploy talking rather than a list somebody maintained. Admin Settings names the missing files; a critical dashboard alert links to it, because the failure mode is that nobody looks. Membership is compared, not the highest number, so 015-applied-while-014-is-not shows as pending. If 016 itself is absent the panel says the database is behind rather than showing a green tick -- the check cannot quietly pass by never having been installed. |
